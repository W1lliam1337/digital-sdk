...
#define FT_CEIL(X)  (((X + 63) & -64) / 64)
	...
	{
		FT_Error error = FT_New_Memory_Face(ft_library, static_cast<uint8_t*>(cfg.FontData),
		                                    static_cast<uint32_t>(cfg.FontDataSize), static_cast<uint32_t>(cfg.FontNo),
		                                    &Face);
		if (error != 0)
			return false;
		...
		SetPixelHeight(static_cast<uint32_t>(cfg.SizePixels));
	...
	void FreeTypeFont::SetPixelHeight(int pixel_height)
	{
		...
		req.width = 0;
		req.height = static_cast<uint32_t>(pixel_height) * 64;
		...
		Info.PixelHeight = static_cast<uint32_t>(pixel_height);
		Info.Ascender = static_cast<float>(FT_CEIL(metrics.ascender));
		Info.Descender = static_cast<float>(FT_CEIL(metrics.descender));
		Info.LineSpacing = static_cast<float>(FT_CEIL(metrics.height));
		Info.LineGap = static_cast<float>(FT_CEIL(metrics.height - metrics.ascender + metrics.descender));
		Info.MaxAdvanceWidth = static_cast<float>(FT_CEIL(metrics.max_advance));
	}
	...
	const FT_Bitmap* FreeTypeFont::RenderGlyphAndGetInfo(GlyphInfo* out_glyph_info)
	{
		...
		out_glyph_info->Width = static_cast<int>(ft_bitmap->width);
		out_glyph_info->Height = static_cast<int>(ft_bitmap->rows);
		out_glyph_info->OffsetX = Face->glyph->bitmap_left;
		out_glyph_info->OffsetY = -Face->glyph->bitmap_top;
		out_glyph_info->AdvanceX = static_cast<float>(FT_CEIL(slot->advance.x));

		return ft_bitmap;
	}

	void FreeTypeFont::BlitGlyph(const FT_Bitmap* ft_bitmap, uint8_t* dst, uint32_t dst_pitch,
...
bool ImFontAtlasBuildWithFreeType(FT_Library ft_library, ImFontAtlas* atlas, unsigned int extra_flags)
{
			...
			if (int entries_32 = *it)
				for (int bit_n = 0; bit_n < 32; bit_n++)
					if (entries_32 & (1 << bit_n))
					{
						...
						src_glyph.Codepoint = static_cast<ImWchar>(((it - it_begin) << 5) + bit_n);
	...
	buf_bitmap_buffers.push_back(static_cast<unsigned char*>(IM_ALLOC(BITMAP_BUFFERS_CHUNK_SIZE)));
	...
	for (int src_i = 0; src_i < src_tmp_array.Size; src_i++)
	{
		...
		for (int glyph_i = 0; glyph_i < src_tmp.GlyphsList.Size; glyph_i++)
		{
			...
			src_tmp.Font.BlitGlyph(ft_bitmap, src_glyph.BitmapData, src_glyph.Info.Width * 1,
			                       multiply_enabled ? multiply_table : nullptr);

			src_tmp.Rects[glyph_i].w = static_cast<stbrp_coord>(src_glyph.Info.Width + padding);
			src_tmp.Rects[glyph_i].h = static_cast<stbrp_coord>(src_glyph.Info.Height + padding);
	...
	for (int src_i = 0; src_i < src_tmp_array.Size; src_i++)
	{
		...
		const float font_off_y = cfg.GlyphOffset.y + static_cast<float>((int)(dst_font->Ascent + 0.5f));
		...
		for (int glyph_i = 0; glyph_i < src_tmp.GlyphsCount; glyph_i++)
		{
			...
			size_t blit_src_stride = static_cast<size_t>(src_glyph.Info.Width);
			size_t blit_dst_stride = static_cast<size_t>(atlas->TexWidth);
			...
			if (char_advance_x_org != char_advance_x_mod)
				char_off_x += cfg.PixelSnapH
					              ? static_cast<float>((int)((char_advance_x_mod - char_advance_x_org) * 0.5f))
			...
			float u0 = (tx) / static_cast<float>(atlas->TexWidth);
			float v0 = (ty) / static_cast<float>(atlas->TexHeight);
			float u1 = (tx + info.Width) / static_cast<float>(atlas->TexWidth);
			float v1 = (ty + info.Height) / static_cast<float>(atlas->TexHeight);
			dst_font->AddGlyph(static_cast<ImWchar>(src_glyph.Codepoint), x0, y0, x1, y1, u0, v0, u1, v1,
			                   char_advance_x_mod);
		}
...
static void* GImFreeTypeAllocatorUserData = nullptr;

// FreeType memory allocation callbacks
static void* FreeType_Alloc(FT_Memory /*memory*/, long size)
{
	return GImFreeTypeAllocFunc(static_cast<size_t>(size), GImFreeTypeAllocatorUserData);
}
...
static void* FreeType_Realloc(FT_Memory /*memory*/, long cur_size, long new_size, void* block)
{
	...
	if (block == nullptr)
		return GImFreeTypeAllocFunc(static_cast<size_t>(new_size), GImFreeTypeAllocatorUserData);
	...
	{
		void* new_block = GImFreeTypeAllocFunc(static_cast<size_t>(new_size), GImFreeTypeAllocatorUserData);
		memcpy(new_block, block, static_cast<size_t>(cur_size));
		GImFreeTypeFreeFunc(block, GImFreeTypeAllocatorUserData);
		return new_block;
	}
