...
bool chams::draw_model( i_model_render* model_render, void* edx, void* context, draw_model_state_t& state, model_render_info_t& info, matrix_t* bones ) {
	...
	if ( is_player && player->is_player( ) && player != ctx::local( ) && player->get_team( ) != ctx::local( )->get_team( ) ) {
		if ( g_cfg.m_esp.m_player[1].m_chams ) {
			...
			if ( g_cfg.m_esp.m_player[1].m_invisible_chams ) {
				...
				hooks::og::m_draw_model_execute( model_render, edx, context, state, info, bones );
			}
