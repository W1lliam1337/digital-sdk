#include "events.h"
#include "../logs system/logs.h"

void c_events::init()
{
	g_sdk.m_interfaces.m_event_manager->add_listener(this, _("player_hurt"), false);
	g_sdk.m_interfaces.m_event_manager->add_listener(this, _("player_death"), false);
}
...
{
	const auto user_id = g_sdk.m_interfaces.m_engine->get_player_for_user_id(event->get_int(_("userid")));
	const auto attacker = g_sdk.m_interfaces.m_engine->get_player_for_user_id(event->get_int(_("attacker")));

	if (strcmp(event_name, _("player_hurt")))
		return;

	const auto hurt_player = static_cast<c_base_player*>(g_sdk.m_interfaces.m_entity_list->get_client_entity(user_id));
	...
	if (attacker == g_sdk.m_interfaces.m_engine->get_local_player())
	{
		player_info_t info{};
