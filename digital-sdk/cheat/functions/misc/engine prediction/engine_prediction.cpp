...
{
	static auto prediction_player = c_utils::find_sig(g_sdk.m_modules.m_client_dll, _("8B 47 40 A3")) + 4;
	static auto prediction_seed = c_utils::find_sig(g_sdk.m_modules.m_client_dll, _("0F 5B C0 89 35")) + 5;
...
void c_engine_prediction::begin()
{
	...
	*reinterpret_cast<c_user_cmd**>(reinterpret_cast<std::uintptr_t>(g_sdk.m_local()) + 0x3298) = g_sdk.m_packet_data.m_cmd; //set(player + 0x3298, cmd);
	...
	g_sdk.m_packet_data.m_cmd->m_buttons.add(*reinterpret_cast<int*>(reinterpret_cast<std::uintptr_t>(g_sdk.m_local()) + 0x3344));
	g_sdk.m_packet_data.m_cmd->m_buttons.remove(*reinterpret_cast<int*>(reinterpret_cast<std::uintptr_t>(g_sdk.m_local()) + 0x3340));
