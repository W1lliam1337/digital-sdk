...
class c_legit_bot : public c_singleton<c_legit_bot>
{
	[[nodiscard]] bool crosshair_on_enemy(const qangle_t view_angles) const
	{
		const auto& start_pos = g_sdk.m_local()->get_shoot_pos();
		vec3_t direction;
		c_math::get()->angle_vectors(view_angles + g_sdk.m_local()->get_aim_punch_angle(), &direction);
		...
		g_sdk.m_interfaces.m_trace->trace_ray(ray, mask_shot | contents_grate, &filter, &exit_trace);
