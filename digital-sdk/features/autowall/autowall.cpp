...
void c_auto_wall::scale_damage(float& damage) const
{
	/* @ref: https://github.com/perilouswithadollarsign/cstrike15_src/blob/master/game/server/player.cpp#L1194 */
	auto is_armored = [](const c_base_player* e_player, const int e_hitgroup)
	{
	...
	if (!is_armored(m_fire_bullet_data.m_enter_trace.m_entity, m_fire_bullet_data.m_enter_trace.m_hit_group))
		return;
...
bool c_auto_wall::simulate_fire_bullet()
{
	/* @ida:
