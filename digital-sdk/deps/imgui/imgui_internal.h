...
enum ImGuiItemStatusFlags_ {
    ...
    ImGuiItemStatusFlags_ToggledSelection = 1
        << 3, // Set when Selectable(), TreeNode() reports toggling a selection. We can't report "Selected", only state
...
enum ImGuiButtonFlagsPrivate_ {
    ...
    ImGuiButtonFlags_AlignTextBaseLine = 1
        << 15, // vertically align button to match text baseline - ButtonEx() only // FIXME: Should be removed and
...
enum ImGuiInputFlags_ {
    ...
    ImGuiInputFlags_RouteUnlessBgFocused = 1
        << 13, // Global routes will not be applied if underlying background/void is focused (== no Dear ImGui windows
...
enum ImGuiScrollFlags_ {
    ...
    ImGuiScrollFlags_KeepVisibleEdgeY = 1
        << 1, // If item is not visible: scroll as little as possible on Y axis to bring item back into view [default
...
enum ImGuiNavMoveFlags_ {
    ...
    ImGuiNavMoveFlags_AllowCurrentNavId = 1
        << 4, // Allow scoring and considering the current NavId as a move target candidate. This is used when the move
    ...
    ImGuiNavMoveFlags_AlsoScoreVisibleSet = 1
        << 5, // Store alternate result in NavMoveResultLocalVisible that only comprise elements that are already fully
...
enum ImGuiOldColumnFlags_ {
    ...
    ImGuiOldColumnFlags_GrowParentContentsSize = 1
        << 4, // (WIP) Restore pre-1.51 behavior of extending the parent window contents size but _without affecting the
...
struct ImGuiContext {
    ...
    ImGuiID NavActivatePressedId; // ~~ IsKeyPressed(ImGuiKey_Space) || IsKeyPressed(ImGuiKey_NavGamepadActivate) ?
                                  // NavId : 0 (no repeat)
    ImGuiID NavActivateInputId;   // ~~ IsKeyPressed(ImGuiKey_Enter) || IsKeyPressed(ImGuiKey_NavGamepadInput) ? NavId :
                                  // 0; ImGuiActivateFlags_PreferInput will be set and NavActivateId will be 0.
...
namespace ImGui {
    ...
    IMGUI_API void LogBegin( ImGuiLogType type,
                             int auto_open_depth ); // -> BeginCapture() when we design v2 api, for now stay under the
                                                    // radar by using the old name.
    ...
    // ahead).
    // - Input queries can then read input by specifying ImGuiKeyOwner_Any (== 0), ImGuiKeyOwner_None (== -1) or a
    // custom ID.
    ...
    // - Route is granted to a single owner. When multiple requests are made we have policies to select the winning
    // route.
    ...
    IMGUI_API int
    TableGetHoveredColumn( ); // May use (TableGetColumnFlags() & ImGuiTableColumnFlags_IsHovered) instead. Return
                              // hovered column. return -1 when table is not hovered. return columns_count if the unused
                              // space at the right of visible columns is hovered.
