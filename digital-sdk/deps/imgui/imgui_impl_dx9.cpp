...
static ImGui_ImplDX9_Data* ImGui_ImplDX9_GetBackendData( ) {
    return ImGui::GetCurrentContext( )
        ? static_cast< ImGui_ImplDX9_Data* >( ImGui::GetIO( ).BackendRendererUserData )
        : nullptr;
}
...
void ImGui_ImplDX9_RenderDrawData( ImDrawData* draw_data ) {
    ...
    if ( !bd->pVB || bd->VertexBufferSize < draw_data->TotalVtxCount ) {
        ...
        if ( bd->pd3dDevice->CreateVertexBuffer( bd->VertexBufferSize * sizeof( CUSTOMVERTEX ),
                                                 ...
                                                 D3DPOOL_DEFAULT, &bd->pVB, nullptr
            ) < 0 )
            return;
    }
    if ( !bd->pIB || bd->IndexBufferSize < draw_data->TotalIdxCount ) {
        ...
        if ( bd->pd3dDevice->CreateIndexBuffer( bd->IndexBufferSize * sizeof( ImDrawIdx ),
                                                ...
                                                D3DPOOL_DEFAULT, &bd->pIB, nullptr
            ) < 0 )
            return;
    }
    ...
    ImDrawIdx* idx_dst;
    if ( bd->pVB->Lock( 0, draw_data->TotalVtxCount * sizeof( CUSTOMVERTEX ), ( void** )&vtx_dst, D3DLOCK_DISCARD
        ) < 0 ) {
        d3d9_state_block->Release( );
        return;
    }
    if ( bd->pIB->Lock( 0, draw_data->TotalIdxCount * sizeof( ImDrawIdx ), ( void** )&idx_dst, D3DLOCK_DISCARD ) < 0 ) {
        bd->pVB->Unlock( );
        d3d9_state_block->Release( );
        return;
    }
    ...
    for ( int n = 0; n < draw_data->CmdListsCount; n++ ) {
        ...
        for ( int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++ ) {
            ...
            if ( pcmd->UserCallback != nullptr ) {
                ...
                const RECT r = { static_cast< LONG >( clip_min.x ), static_cast< LONG >( clip_min.y ),
                                 static_cast< LONG >( clip_max.x ), static_cast< LONG >( clip_max.y ) };
                const auto texture = static_cast< LPDIRECT3DTEXTURE9 >( pcmd->GetTexID( ) );
...
static bool ImGui_ImplDX9_CreateFontsTexture( ) {
    ...
    if ( io.Fonts->TexPixelsUseColors ) {
        auto dst_start = static_cast< ImU32* >( ImGui::MemAlloc( ( size_t )width * height * bytes_per_pixel ) );
        for ( ImU32* src = ( ImU32* )pixels,* dst = dst_start,* dst_end = dst_start + static_cast< size_t >( width ) *
                       height; dst < dst_end; src++, dst++ )
